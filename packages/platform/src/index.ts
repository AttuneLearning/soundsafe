...
export type { KeyCombo, KeybindOptions, KeybindService, Unsubscribe } from './keybind.ts';

export interface Platform {
  readonly name: 'web' | 'tauri' | 'mobile';
  readonly keybind: import('./keybind.ts').KeybindService;
}

export type CreatePlatform = () => Platform;

// Re-export the web factory + the React context bits. The bundler will swap
// `../web/index.ts` for `../tauri/index.ts` (etc.) when SOUNDSAFE_PLATFORM
